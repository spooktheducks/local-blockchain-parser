...
func GetPushdataBytesFromInputScript(scriptBytes []byte) ([]byte, error) {
	...
	for i := 0; i < len(scriptBytes); {
		...
		if op >= OP_DATA_1 && op <= OP_DATA_75 {
			...
			length := uint8(scriptBytes[i+1])
			data := scriptBytes[i+2 : i+2+int(length)]
			outData = append(outData, data...)

			i += 1 + int(length)
		} else if op == OP_PUSHDATA2 {
			length := binary.LittleEndian.Uint16(scriptBytes[i+1:])
			data := scriptBytes[i+3 : i+3+int(length)]
			outData = append(outData, data...)

			i += 3 + int(length)
		} else if op == OP_PUSHDATA4 {
			length := binary.LittleEndian.Uint32(scriptBytes[i+1:])
			data := scriptBytes[i+5 : i+5+int(length)]
			outData = append(outData, data...)
...
func GetNonOPBytesFromInputScript(scriptBytes []byte) ([]byte, error) {
	outData := []byte{}
	for i := 0; i < len(scriptBytes); {
		op := scriptBytes[i]

		if op >= OP_DATA_1 && op <= OP_DATA_75 {
			length := opcodeArray[op].length - 1
			data := scriptBytes[i+1 : i+1+length]
			outData = append(outData, data...)
