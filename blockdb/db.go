...
func (db *BlockDB) IndexDATFileTransactions(startBlock, endBlock uint64, force bool) error {
	for i := int(startBlock); i < int(endBlock)+1; i++ {
		...
		if !force {
			isIndexed, err := db.CheckIfTransactionsIndexed(datFilename)
			if err != nil {
				return err
			} else if isIndexed {
		...
		err = db.SetTransactionsIndexed(datFilename)
		if err != nil {
			return err
		}
	}
...
func (db *BlockDB) CheckIfTransactionsIndexed(filename string) (bool, error) {
	var indexed bool
	err := db.store.Update(func(boltTx *bolt.Tx) error {
		bucket, err := boltTx.CreateBucketIfNotExists([]byte(BucketTransactionsIndexedBlocks))
		if err != nil {
			return err
		}
...
func (db *BlockDB) SetTransactionsIndexed(filename string) error {
	err := db.store.Update(func(boltTx *bolt.Tx) error {
		bucket, err := boltTx.CreateBucketIfNotExists([]byte(BucketTransactionsIndexedBlocks))
		if err != nil {
			return err
